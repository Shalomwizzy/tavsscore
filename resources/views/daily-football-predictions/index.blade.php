        ...
        @else
            ...
            @foreach($predictions as $prediction)
                ...
                <tr>
                    ...
                    <td><div class="dfp-match">{{ $match?->home_team ?? '?' }} vs {{ $match?->away_team ?? '?' }}</div><div class="dfp-meta">{{ \App\Support\LeagueCoverage::formatName($match?->league, $match?->league_country) }}@if($match?->home_score !== null) · {{ $match->home_score }}–{{ $match->away_score }}@endif</div></td>
