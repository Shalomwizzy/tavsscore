...
<style>
    .picks-hero {
        ...
        background:
            radial-gradient(ellipse 70% 60% at 50% -10%, rgba(16,185,129,.10), transparent),
            radial-gradient(ellipse 50% 40% at 90% 80%, rgba(59,130,246,.07), transparent);
    ...
    .picks-title .accent { color: #6ee7b7; }
    ...
    .pick-card:hover { border-color: rgba(16,185,129,.3); transform: translateY(-2px); }
    ...
    .rank-1 { background: rgba(16,185,129,.15); border: 1px solid rgba(16,185,129,.3);  color: #6ee7b7; }
    ...
    .pick-callout.callout-win  { background: linear-gradient(135deg, rgba(16,185,129,.18), rgba(16,185,129,.08)); border-color: rgba(16,185,129,.4); }
    ...
    .pick-callout-label { font-size: .68rem; font-weight: 700; color: rgba(110,231,183,.6); text-transform: uppercase; letter-spacing: .06em; margin-bottom: .25rem; }
    .pick-callout-value { font-size: 1.25rem; font-weight: 900; color: #6ee7b7; letter-spacing: -.01em; }
    ...
    .btts-bar-outer { height: 8px; border-radius: 999px; background: var(--surface); border: 1px solid var(--border); overflow: hidden; margin-bottom: .45rem; }
    .btts-bar-fill  { height: 100%; background: linear-gradient(90deg, #10b981, #3b82f6); }
    ...
    .hiw-num   { width: 28px; height: 28px; border-radius: 50%; background: rgba(16,185,129,.15); border: 1px solid rgba(16,185,129,.3); color: #6ee7b7; font-size: .75rem; font-weight: 900; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 1px; }
    ...
    <div class="wrap">
        ...
        <div class="accuracy-strip">
            <div style="display:flex; align-items:center; gap:1.1rem;">
                @php
                    $pct = $accuracy['pct'];
                    $cls = $pct >= 60 ? 'good' : ($pct >= 50 ? 'ok' : 'low');
                @endphp
                <div class="accuracy-pct {{ $cls }}">{{ $pct }}%</div>
                <div>
                    <div class="accuracy-label">
                        <strong>7-Day GG Accuracy</strong>
        ...
        <div class="picks-grid">
            ...
            @php
                ...
                $rankLabel = match($pick['rank']) { 1 => '👑 Top GG Pick', 2 => '⚽ GG #2', 3 => '⚽ GG #3', default => '⚽ GG #' . $pick['rank'] };
                $tips = $pick['tips'] ?? [];
            ...
            <div class="pick-card {{ $resultClass }}">
                <div class="pick-card-top">
                    ...
                    <div class="pick-callout {{ $calloutClass }}">
                        <div>
                            ...
                            <div class="pick-callout-value">⚽ Both Teams to Score (GG)</div>
                        </div>
                        <div style="text-align:right;">
                            ...
                            @else
                                ...
                                <div style="font-size:.7rem; color:rgba(110,231,183,.6); text-transform:uppercase; letter-spacing:.06em; margin-bottom:.2rem;">Confidence</div>
                                <div style="font-size:1.4rem; font-weight:900; color:#6ee7b7;">{{ $pick['confidence_pct'] }}%</div>
                                @endif
                            @endif
                        </div>
                    </div>
                ...
                <div class="prob-section">
                    ...
                    <div style="margin-top:.75rem;">
                        <div style="font-size:.68rem; color:var(--text-dim); font-weight:700; text-transform:uppercase; letter-spacing:.06em; margin-bottom:.4rem;">BTTS Probability</div>
                        <div class="btts-bar-outer">
                            <div class="btts-bar-fill" style="width:{{ min(100, $pick['btts']) }}%;"></div>
                        </div>
                        <div style="font-size:.68rem; color:#6ee7b7; font-weight:700;">{{ round($pick['btts']) }}% chance both teams score</div>
                    </div>
                    @endif
                </div>
                @endif
        ...
        <div class="how-it-works">
            <div class="hiw-title">How GG Picks Work</div>
            <div class="hiw-steps">
                <div class="hiw-step">
