...
<style>
    ...
    .daily-desk-controls { display:flex; align-items:center; flex-wrap:wrap; gap:.45rem; margin: .85rem 0; }.daily-desk-controls form:last-child { margin-left:auto; }.daily-desk-input { width:auto; padding:.42rem .52rem; }
...
<div class="daily-desk">
    ...
    <nav class="daily-desk-controls" aria-label="Prediction date">
        <form method="POST" action="{{ route('admin.daily-football-predictions.settle-past') }}" onsubmit="return confirm('Check and settle every past pending Football and Tennis prediction using verified results? Matches without a confirmed final score will stay pending.');">
            @csrf
            <button type="submit" class="btn-a btn-green">✓ Settle past pending</button>
        </form>
