...
@section('title', 'Draw Picks Admin')
@section('page-title', 'Draw Picks')
...
<div class="page-hd">
    ...
    <div style="display:flex; gap:.5rem; flex-wrap:wrap;">
        <form method="POST" action="{{ route('admin.draw-picks.refresh') }}"
            ...
            @csrf
            <button type="submit" class="btn-a" style="background:linear-gradient(135deg,#92400e,#78350f);color:#fcd34d;">🤝 Re-select Today</button>
        </form>
        <a href="{{ route('draw-picks.index') }}" target="_blank" class="btn-a btn-blue">↗ View Public Page</a>
    </div>
</div>
...
{{-- Today --}}
<div class="a-card" style="margin-bottom:1.25rem; border-color:rgba(245,158,11,.25);">
    <div class="page-hd" style="margin-bottom:.875rem;">
        <span style="font-weight:700; font-size:.9rem; color:#fff;">
            🤝 Today's Draw Picks
            <span style="background:rgba(245,158,11,.15);color:#fcd34d;font-size:.65rem;padding:1px 8px;border-radius:999px;margin-left:.4rem;">{{ $todayPicks->count() }}/5</span>
        </span>
    ...
    @if($todayPicks->isEmpty())
        <div style="text-align:center; padding:1.75rem; color:var(--dim); font-size:.85rem;">
            No draw picks selected today.<br>
        ...
        <table class="a-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:#fcd34d; font-weight:700;">{{ $pick->draw_prob ? round($pick->draw_prob) . '%' : '-' }}</td>
                    ...
                    <td>
                        <form method="POST" action="{{ route('admin.picks.regenerate', $pick) }}"
                            ...
                            @csrf
                            <input type="hidden" name="type" value="draw">
...
<div class="a-card">
    ...
    @if($history->isEmpty())
        <div style="text-align:center; padding:1.75rem; color:var(--dim); font-size:.85rem;">No previous draw picks recorded yet.</div>
    @else
                ...
                <table class="a-table" style="font-size:.78rem;">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="color:#fcd34d; font-weight:700;">Draw</td>
...
<div style="margin-top:1.25rem; padding:.875rem 1rem; border-radius:8px; background:rgba(245,158,11,.07); border:1px solid rgba(245,158,11,.18); font-size:.74rem; color:#fcd34d;">
    <strong>ℹ️ How draw picks work:</strong>
    Draw picks auto-select at <code style="background:rgba(255,255,255,.1);padding:1px 5px;border-radius:3px;">06:00</code> via <code style="background:rgba(255,255,255,.1);padding:1px 5px;border-radius:3px;">picks:select</code>.
    Only predictions where all 3 AI engines independently agree on "Draw" with ≥60% confidence qualify.
