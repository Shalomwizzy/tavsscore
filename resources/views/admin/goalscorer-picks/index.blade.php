...
<div style="font-size:.78rem; color:var(--dim); background:var(--card,#111826); border:1px solid var(--border); border-radius:10px; padding:.7rem .9rem; margin-bottom:1.25rem;">
    Computed live from player scoring rates vs opponent defence (no stored table). Anytime scorer is ~50 to 60% even for elite strikers, higher-odds, not bankers. Needs <code>player_statistics</code> populated (<code>stats:fetch-players</code>).
</div>
    ...
    <table class="a-table" style="min-width:720px;">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ !empty($p['two_plus']) ? number_format($p['two_plus'],0).'%' : 'N/A' }}</td>
