...
<div class="risk-page">
    ...
    @if($preview)
        ...
        <section class="risk-ticket"><div class="risk-ticket-top"><div class="risk-platform"><span>📊</span>{{ $preview['title'] }}</div><span class="risk-odds">Target 15.00× to 1,500×</span></div><details class="risk-legs" open><summary>Model selections <span>{{ count($preview['selections'] ?? []) }} legs</span></summary>@foreach(($preview['selections'] ?? []) as $leg)<div class="risk-leg"><div class="risk-leg-main"><span>@if(($leg['sport'] ?? 'football') === 'tennis')🎾 @elseif(!empty($leg['home_logo']))<img src="{{ $leg['home_logo'] }}" alt="" style="width:20px;height:20px;object-fit:contain;vertical-align:middle">@endif {{ $leg['home'] }} <b style="color:#64748b">vs</b> @if(!empty($leg['away_logo']) && ($leg['sport'] ?? 'football') === 'football')<img src="{{ $leg['away_logo'] }}" alt="" style="width:20px;height:20px;object-fit:contain;vertical-align:middle">@endif {{ $leg['away'] }}</span><span class="risk-leg-prob">{{ number_format((float)($leg['model_prob'] ?? 0),0) }}%</span></div><div class="risk-leg-market">@nodash($leg['market'] ?? 'Model market')</div></div>@endforeach</details></section>
    @endif
