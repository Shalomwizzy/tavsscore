...
<style>
    ...
    .risk-board{padding:1rem;border:1px solid rgba(251,113,133,.22);border-radius:18px;background:linear-gradient(145deg,rgba(43,18,35,.76),rgba(14,25,41,.9))}.risk-board-top{display:flex;justify-content:space-between;gap:.7rem;align-items:center;margin-bottom:.8rem}.risk-board-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.5rem}.risk-pred{display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:.48rem;align-items:center;min-height:58px;padding:.55rem .62rem;border:1px solid rgba(255,255,255,.075);border-radius:11px;background:rgba(6,12,23,.42)}.risk-pred-logo{display:flex;align-items:center;gap:.18rem}.risk-pred-logo img{width:23px;height:23px;object-fit:contain}.risk-pred-match{overflow:hidden;color:#eef2ff;font-size:.7rem;font-weight:800;white-space:nowrap;text-overflow:ellipsis}.risk-pred-market{margin-top:.14rem;color:#94a3b8;font-size:.61rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.risk-pred-prob{color:#fecdd3;font-size:.68rem;font-weight:900}.risk-board-more{margin-top:.65rem;text-align:center;color:#94a3b8;font-size:.66rem}.risk-board-more summary{cursor:pointer;list-style:none}.risk-board-more summary::-webkit-details-marker{display:none}
...
<div class="risk-page">
    ...
    @if($preview)
        ...
        <section class="risk-board"><div class="risk-board-top"><div class="risk-platform"><span>📊</span>{{ $preview['title'] }}</div><span class="risk-odds">15.00× to 1,500×</span></div><div class="risk-board-grid">@foreach(collect($preview['selections'] ?? [])->take(6) as $leg)<article class="risk-pred"><div class="risk-pred-logo">@if(($leg['sport'] ?? 'football') === 'tennis')🎾@else @if(!empty($leg['home_logo']))<img src="{{ $leg['home_logo'] }}" alt="">@endif @if(!empty($leg['away_logo']))<img src="{{ $leg['away_logo'] }}" alt="">@endif @endif</div><div style="min-width:0"><div class="risk-pred-match">{{ $leg['home'] }} vs {{ $leg['away'] }}</div><div class="risk-pred-market">@nodash($leg['market'] ?? 'Model market')</div></div><div class="risk-pred-prob">{{ number_format((float)($leg['model_prob'] ?? 0),0) }}%</div></article>@endforeach</div>@if(count($preview['selections'] ?? []) > 6)<details class="risk-board-more"><summary>View {{ count($preview['selections']) - 6 }} more selections</summary><div class="risk-board-grid" style="margin-top:.55rem">@foreach(collect($preview['selections'])->slice(6) as $leg)<article class="risk-pred"><div class="risk-pred-logo">{{ ($leg['sport'] ?? 'football') === 'tennis' ? '🎾' : '⚽' }}</div><div style="min-width:0"><div class="risk-pred-match">{{ $leg['home'] }} vs {{ $leg['away'] }}</div><div class="risk-pred-market">@nodash($leg['market'] ?? 'Model market')</div></div><div class="risk-pred-prob">{{ number_format((float)($leg['model_prob'] ?? 0),0) }}%</div></article>@endforeach</div></details>@endif</section>
    @endif
