...
class Over25AdminController extends Controller
{
    ...
    public function rebuild(): RedirectResponse
    {
        try {
            ...
            $picks = $this->predictionService->selectOver25Picks();
            if ($picks->isNotEmpty()) Artisan::call('picks:notify', ['--type' => 'over25', '--force' => true]);
            return redirect()->route('admin.over25.index')->with('success', "Latest fixtures and model boards rebuilt. {$picks->count()} Over 2.5 pick(s) qualified and were sent where available.");
        } catch (\Throwable $exception) {
            return redirect()->route('admin.over25.index')->with('error', $exception->getMessage());
        }
    }
}
